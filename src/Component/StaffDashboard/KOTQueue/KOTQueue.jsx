...
//                           <td className="kot-table-td px-4 py-3">
//                             <span
//                               className={`kot-category-badge d-inline-flex align-items-center px-2 py-1 rounded-pill small fw-medium ${order.item_name.toLowerCase().includes("cola")
...
//                           <td className="kot-table-td px-4 py-3">
//                             <span
//                               className={`kot-status-badge d-inline-flex align-items-center px-2 py-1 rounded-pill small fw-medium ${order.item_status === "pending"
        ...
        setOrders(prevOrders =>
          prevOrders.map(order =>
            order.item_id === itemId
              ? { ...order, item_status: status }
              : order
          )
        );
        console.log(`Order ${itemId} status updated to ${status}`);
      } else {
            ...
            <div className="kot-active-kots-section">
              <div className="kot-section-header d-flex justify-content-between align-items-center mb-4">
                ...
                <div className="kot-section-filters d-flex align-items-center gap-3">
                  ...
                  <select className="kot-category-select form-select px-5 py-2 border rounded-1">
                    <option>All Categories</option>
                    <option>Food</option>
                    <option>Beverages</option>
                  </select>
              ...
              <div className="kot-printer-grid row g-4">
                ...
                <div className="kot-printer-card col-lg-6">
                  <div className="kot-card bg-white rounded shadow-sm border border-light p-4 h-100">
                    ...
                    <div className="kot-order-list">
                      {orders.filter(order => !order.item_name.toLowerCase().includes("cola")).length === 0 ? (
                        <div className="text-center py-4 text-muted">No food orders</div>
                      ) : (
                        orders
                          .filter(order => !order.item_name.toLowerCase().includes("cola"))
                          .map((order, index) => (
                            <div key={index} className="kot-order-card border border-light rounded-1 p-3 mb-3">
                              <div className="kot-order-header d-flex justify-content-between align-items-center mb-2">
                                <span className="kot-order-id fw-medium text-dark">#{order.order_number}</span>
                                <span className={`kot-order-status small px-2 py-1 rounded-pill fw-medium ${order.item_status === "pending" ? "bg-warning text-dark" : "bg-success-100 text-success-800"
                                  }`}>
                                  {order.item_status === "pending" ? "Sent to Kitchen" : "Completed"}
                                </span>
                              </div>
                              <div className="kot-order-items small text-muted">
                                {order.item_name} × {order.quantity}
                ...
                <div className="kot-printer-card col-lg-6">
                  <div className="kot-card bg-white rounded shadow-sm border border-light p-4 h-100">
                    ...
                    <div className="kot-order-list">
                      {orders.filter(order => order.item_name.toLowerCase().includes("cola")).length === 0 ? (
                        <div className="text-center py-4 text-muted">No beverage orders</div>
                      ) : (
                        orders
                          .filter(order => order.item_name.toLowerCase().includes("cola"))
                          .map((order, index) => (
                            <div key={index} className="kot-order-card border border-light rounded-1 p-3 mb-3">
                              <div className="kot-order-header d-flex justify-content-between align-items-center mb-2">
                                ...
                                <span className={`kot-order-status small px-2 py-1 rounded-pill fw-medium ${order.item_status === "pending" ? "bg-warning text-dark" : "bg-success-100 text-success-800"
                                  ...
                                  {order.item_status === "pending" ? "Sent to Bar" : "Completed"}
                                </span>
                              </div>
                              <div className="kot-order-items small text-muted">
                                {order.item_name} × {order.quantity}
                ...
                <div className="kot-stats-grid row">
                  <div className="kot-stat-col col-md-3 text-center">
                    ...
                    <div className="kot-stat-value text-dark fs-3 fw-bold">
                      {orders.filter(order => order.item_status !== "pending").length}
                    </div>
                    <div className="kot-stat-label small text-muted">Completed Today</div>
                  </div>
                  <div className="kot-stat-col col-md-3 text-center">
